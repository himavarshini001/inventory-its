...
// ─── Mock Data ────────────────────────────────────────────────────────────────
const REGIONS = ["Bengaluru", "Pune"];

const INVENTORY_CATEGORIES = ["3PL AA/CP", "P3 Accessories", "3PL GAS"];

const STATUS_LIST = ["Available", "Reserved", "Allocated", "Returned", "Lost", "Damaged", "Repair", "Retired"];
...
const ENGINEERS = ["Alice Kumar", "Bob Sharma", "Carol Nair", "David Menon", "Eve Singh"];
...
// ─── P3 Logo (original mark, navy colorway) ────────────────────────────────────
...
const Icon = ({ name, size = 16, color = "currentColor" }) => {
  const icons = {
    dashboard: <svg width={size} height={size} fill="none" stroke={color} strokeWidth="2" viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>,
    inventory: <svg width={size} height={size} fill="none" stroke={color} strokeWidth="2" viewBox="0 0 24 24"><path d="M20 7H4a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2z"/><path d="M16 3H8l-2 4h12l-2-4z"/></svg>,
    barcode: <svg width={size} height={size} fill="none" stroke={color} strokeWidth="2" viewBox="0 0 24 24"><path d="M3 5v14M7 5v14M11 5v14M15 5v14M19 5v14M21 5v14"/></svg>,
    allocate: <svg width={size} height={size} fill="none" stroke={color} strokeWidth="2" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></svg>,
    return: <svg width={size} height={size} fill="none" stroke={color} strokeWidth="2" viewBox="0 0 24 24"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-4.09"/></svg>,
    audit: <svg width={size} height={size} fill="none" stroke={color} strokeWidth="2" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>,
    report: <svg width={size} height={size} fill="none" stroke={color} strokeWidth="2" viewBox="0 0 24 24"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>,
    bulk: <svg width={size} height={size} fill="none" stroke={color} strokeWidth="2" viewBox="0 0 24 24"><rect x="1" y="4" width="22" height="16" rx="2"/><path d="M1 10h22"/></svg>,
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: "#fff", borderRadius: 12, width: "100%", maxWidth: width, maxHeight: "90vh", overflow: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 24px 16px", borderBottom: "1px solid #e5e7eb" }}>
          ...
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", padding: 4, color: "#6b7280" }}><Icon name="close" size={20} /></button>
        </div>
...
function Input({ label, value, onChange, type = "text", required, options, placeholder }) {
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      {options ? (
        <select value={value} onChange={e => onChange(e.target.value)} style={{ width: "100%", padding: "9px 12px", border: "1.5px solid #d1d5db", borderRadius: 8, fontSize: 14, color: "#111827", background: "#fff" }}>
...
function Btn({ children, onClick, variant = "primary", size = "md", icon, disabled }) {
  ...
  const sizes = { sm: { padding: "6px 12px", fontSize: 12 }, md: { padding: "9px 16px", fontSize: 14 }, lg: { padding: "12px 24px", fontSize: 15 } };
  return (
    <button onClick={onClick} disabled={disabled}
      style={{ display: "inline-flex", alignItems: "center", gap: 6, borderRadius: 8, cursor: disabled ? "not-allowed" : "pointer", fontWeight: 600, opacity: disabled ? 0.5 : 1, ...styles[variant], ...sizes[size] }}>
...
function Dashboard({ inventory, auditLog }) {
  ...
  const byCategory = INVENTORY_CATEGORIES.map(cat => ({
    cat, available: inventory.filter(i => i.category === cat && i.status === "Available").length,
    total: inventory.filter(i => i.category === cat).length,
  }));

  const byRegion = REGIONS.map(r => ({
    r, count: inventory.filter(i => i.region === r).length,
  }));
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 24 }}>
        <StatCard label="Total Inventory" value={inventory.length} color="#2563eb" />
        <StatCard label="Available" value={counts.Available} color="#16a34a" />
        <StatCard label="Allocated" value={counts.Allocated} color="#d97706" />
        <StatCard label="Returned" value={counts.Returned} color="#059669" />
        <StatCard label="Damaged / Lost" value={(counts.Damaged || 0) + (counts.Lost || 0)} color="#dc2626" />
      </div>
    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        ...
        {(role === "Administrator" || role === "Inventory Manager") && (
          <Btn icon="plus" onClick={() => { setForm(emptyForm); setShowAdd(true); }}>Add Item</Btn>
        )}
      </div>
      ...
      {showAdd && (
        <Modal title="Add Inventory Item" onClose={() => setShowAdd(false)}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 8 }}>
            ...
            <Btn onClick={handleAdd} icon="check">Add Item</Btn>
          </div>
        </Modal>
      )}
      ...
      {showEdit && (
        <Modal title="Edit Inventory Item" onClose={() => setShowEdit(null)}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 8 }}>
            ...
            <Btn onClick={handleEdit} icon="check">Save Changes</Btn>
          </div>
        </Modal>
      )}
      ...
      {showAllocate && (
        <Modal title={`Allocate: ${showAllocate.name}`} onClose={() => setShowAllocate(null)} width={440}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
            <Btn variant="secondary" onClick={() => setShowAllocate(null)}>Cancel</Btn>
            <Btn onClick={handleAllocate} icon="allocate" disabled={!allocForm.engineer || !allocForm.project}>Allocate</Btn>
          </div>
        </Modal>
      )}
      ...
      {showReturn && (
        <Modal title={`Return: ${showReturn.name}`} onClose={() => setShowReturn(null)} width={400}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
            <Btn variant="secondary" onClick={() => setShowReturn(null)}>Cancel</Btn>
            <Btn variant="success" onClick={handleReturn} icon="return">Confirm Return</Btn>
          </div>
        </Modal>
      )}
    </div>
    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <Btn icon="barcode" onClick={printBarcodes}>{selected.length > 0 ? `Print ${selected.length} Selected` : "Bulk Print"}</Btn>
          <Btn icon="download" variant="secondary" onClick={() => alert("Exporting barcodes as PDF…")}>Export PDF</Btn>
        </div>
      </div>
...
function AllocationSection({ inventory, setInventory, auditLog, setAuditLog }) {
            ...
            {allocated.map((item, idx) => {
              ...
              return (
                <tr key={item.id} style={{ borderBottom: "1px solid #f3f4f6", background: overdue ? "#fff7ed" : idx % 2 === 0 ? "#fff" : "#fafafa" }}>
                  ...
                  <td style={{ padding: "10px 14px" }}>
                    <span style={{ color: overdue ? "#dc2626" : "#374151", fontWeight: overdue ? 700 : 400 }}>
                      {item.expectedReturn || "—"} {overdue && "⚠ OVERDUE"}
                    </span>
                  </td>
                  <td style={{ padding: "10px 14px", color: "#6b7280" }}>{item.region}</td>
                  <td style={{ padding: "10px 14px" }}><Badge status={item.status} /></td>
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 24 }}>
        <StatCard label="Overdue Returns" value={overdue.length} color="#dc2626" />
        <StatCard label="Due in 7 Days" value={dueSoon.length} color="#d97706" />
        <StatCard label="Low Stock Items" value={lowStock.length} color="#7c3aed" />
      </div>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        ...
        <Btn icon="download" variant="secondary" onClick={() => alert("Exporting report as Excel/CSV/PDF…")}>Export Report</Btn>
      </div>
      ...
      {reportType === "utilization" && (
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: "#f9fafb", borderBottom: "1px solid #e5e7eb" }}>{["Category", "Total", "Allocated", "Available", "Utilization %"].map(h => <th key={h} style={{ padding: "12px 14px", textAlign: "left", fontWeight: 700, color: "#374151" }}>{h}</th>)}</tr></thead>
            <tbody>
      ...
      {reportType === "engineer" && (
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: "#f9fafb", borderBottom: "1px solid #e5e7eb" }}>{["Engineer", "Total Assigned", "Currently Active"].map(h => <th key={h} style={{ padding: "12px 14px", textAlign: "left", fontWeight: 700, color: "#374151" }}>{h}</th>)}</tr></thead>
            <tbody>
      ...
      {reportType === "region" && (
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: "#f9fafb", borderBottom: "1px solid #e5e7eb" }}>{["Region", "Total", "Available", "Allocated"].map(h => <th key={h} style={{ padding: "12px 14px", textAlign: "left", fontWeight: 700, color: "#374151" }}>{h}</th>)}</tr></thead>
            <tbody>
      ...
      {reportType === "missing" && (
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: "#f9fafb", borderBottom: "1px solid #e5e7eb" }}>{["Item ID", "Name", "Status", "Engineer", "Expected Return", "Region"].map(h => <th key={h} style={{ padding: "12px 14px", textAlign: "left", fontWeight: 700, color: "#374151" }}>{h}</th>)}</tr></thead>
            <tbody>
      ...
      <div style={{ background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 12, padding: 20, marginBottom: 20 }}>
        {bulkAction === "status_update" && (
          <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <Btn onClick={handleBulkStatusUpdate} disabled={selected.length === 0}>{selected.length > 0 ? `Update ${selected.length} Selected` : "Select Items Below"}</Btn>
          </div>
        )}
        {bulkAction === "assign" && (
          <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <Btn onClick={handleBulkAssign} disabled={selected.length === 0 || !bulkEngineer} icon="allocate">{selected.length > 0 ? `Assign ${selected.length} Items` : "Select Items Below"}</Btn>
          </div>
        )}
        {bulkAction === "return" && (
          <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <Btn variant="success" onClick={handleBulkReturn} icon="return">{selected.length > 0 ? `Return ${selected.length} Selected` : "Return All Allocated"}</Btn>
          </div>
        )}
        {bulkAction === "barcode" && (
          <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <Btn onClick={handleBulkBarcodeGen} icon="barcode">Generate Barcodes</Btn>
            <Btn variant="secondary" icon="download" onClick={() => alert("Exporting all barcodes as PDF…")}>Export All Barcodes PDF</Btn>
          </div>
        )}
        {bulkAction === "upload" && (
          <div>
            ...
            <div style={{ marginTop: 10 }}>
              <Btn variant="secondary" icon="download" onClick={() => alert("Downloading import template...")}>Download Import Template</Btn>
            </div>
          </div>
        )}
      </div>

      {/* Inventory Table for selection */}
      {bulkAction !== "upload" && (
        <div style={{ overflowX: "auto", background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12 }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
        ...
        <div style={{ width: "100%", maxWidth: 420 }}>
          ...
          <div style={{ background: "#f8fafc", border: "1px solid #e5e7eb", borderRadius: 12, padding: 16 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {MOCK_USERS.map(u => (
                <button key={u.email} onClick={() => handleQuickLogin(u)}
                  style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", background: "#fff", border: "1.5px solid #e5e7eb", borderRadius: 8, cursor: "pointer", textAlign: "left", transition: "border-color 0.15s" }}>
      ...
      <div style={{ width: sidebarOpen ? 220 : 56, background: "#111827", color: "#fff", display: "flex", flexDirection: "column", transition: "width 0.2s", flexShrink: 0 }}>
        ...
        <nav style={{ flex: 1, padding: "8px 8px", overflowY: "auto" }}>
          {NAV.filter(n => allowed.includes(n.key)).map(n => (
            <button key={n.key} onClick={() => setPage(n.key)}
              style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "10px 12px", borderRadius: 8, border: "none", background: page === n.key ? "#1d4ed8" : "none", color: page === n.key ? "#fff" : "#9ca3af", cursor: "pointer", textAlign: "left", fontSize: 13, fontWeight: page === n.key ? 700 : 500, marginBottom: 2, transition: "background 0.15s" }}>
        ...
        {sidebarOpen && (
          <div style={{ padding: "16px", borderTop: "1px solid #1f2937" }}>
            ...
            <button onClick={() => { setCurrentUser(null); setPage("dashboard"); }}
              style={{ width: "100%", padding: "8px 12px", background: "#1f2937", border: "1px solid #374151", borderRadius: 8, color: "#9ca3af", fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "0 24px", height: 56, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          <div style={{ fontSize: 13, color: "#6b7280" }}>
            <span style={{ fontWeight: 700, color: "#111827" }}>{NAV.find(n => n.key === page)?.label}</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
